...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmLambda (params', Simple, expr) -> ScmLambda'(List.map (fun param -> tag_lexical_address_for_var param params env) params', Simple, run expr params env)
      | ScmLambda (params', Opt opt, expr) -> ScmLambda'(List.map (fun param -> tag_lexical_address_for_var param params env) params', Opt opt, run expr params env)
      | ScmApplic (proc, args) ->
